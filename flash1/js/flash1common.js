...
var files_eval = new Array(
	...
	'classes[11]!=0', // hseb current
	'classes[11]!=0', // hseb exam
...
function forceSpecialNumberInput(obj,event)
   {
 var charCode;
 if(event.keyCode) //For IE
   charCode = event.keyCode;
 else if(event.Which)
   charCode = event.Which; // For FireFox
 else
   charCode = event.charCode; // Other Browser
		...
		if (autoFillRequest == true) 
                {
                    ...
                    if (currentPage=="ecd_details.php")
                        {
...
/**  funtion that retrieves the '$_GET[]' parameters in the url for use in javascript 
 *  @param string url i.e localhost/flash/ecd_details?&n=1
 */
