function handleChange(obj){
	...
	var n='';
	for (i=1;i<=5;i++){
		n=obj.id.replace(/[12345]/,i);
		sum += d[n].value*1;
	}
	
	n=obj.id.replace(/[12345]/,'0');
	d[n].value = sum?sum:'';
...
function validation(obj){
	...
	blink(obj.id.replace(/_enroll_|_appeared_exam_|_passed_exam_/,'_appeared_exam_'), obj.id.replace(/_enroll_|_appeared_exam_|_passed_exam_/,'_enroll_'), "Number of students appearing in exam should be less than number of enrollments");
	blink(obj.id.replace(/_enroll_|_appeared_exam_|_passed_exam_/,'_passed_exam_'), obj.id.replace(/_enroll_|_appeared_exam_|_passed_exam_/,'_enroll_'), "Number of students passing exam should be less than number of enrollments");
	
	if (obj.id.indexOf('_enroll_')>=0){
		blinkLarge(new Array(27,28, 63,64, 99,100, 135,136, 171,172));
	}
