function handleChange(obj){
	...
	if (obj.id.indexOf('hsec_')==0){
		...
		if (obj.id.indexOf('pri_')==0) suffix=new Array("fully_trained","hour_trained","first_package","second_package","untrained");
		if (obj.id.indexOf('lsec_')==0) suffix=new Array("fully_trained","first_package","second_package","third_package","untrained");
		if (obj.id.indexOf('sec_')==0) suffix=new Array("fully_trained","first_package","second_package","third_package","untrained");
		
		
		sum=0;
		for (i=0;i<suffix.length;++i){
			t=obj.id.replace(/fully_trained|part_trained|first_package|second_package|third_package|untrained|hour_trained/,suffix[i]);
			sum+=d[t].value*1;
		}
		
		t=obj.id.replace(/fully_trained_|part_trained_|first_package_|second_package_|third_package_|untrained_|hour_trained_/,'');
