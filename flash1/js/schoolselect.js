...
function schoolSelect(o){
	...
	if (o.id=='slist_e' && document.getElementById('merge_with').value!=''){
		var url = 'flash1backend.php?req=sch_merge&s=' + document.getElementById('merge_with').value + '&mw=' + o.value;
...
function schoolClose(){
	...
	if (confirm('Do you want to close '+ document.getElementById('slist_ne').options[index].text) + '?'){
		...
		ajaxRequest('flash1backend.php?req=sch_close&s='+document.getElementById('s').value,function(d){
			vdcChange();
		});
