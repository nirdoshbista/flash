...
if ($req=='distlist'){
	...
	printf('Select district: <select name="d" id="d" onchange="return handlechange(this, event);">');
...
if ($req=='vdclist'){
	if (isset($_GET['distcode'])){
		...
		printf('Select VDC: <select name="v" id="v" onchange="return handlechange(this, event);">');
...
if ($req=='schoollist'){
	...
	//$result = mysql_query(sprintf('select *, TRIM(nm_sch) as schoolname from mast_schoollist where dist_code="%s" and vdc_code="%s" order by schoolname',$dist, $vdc));
	//$rows = mysql_fetch_all($result);

	//echo 'Select School: <select name="s" id="s">';
	//printf('<option value="0">- School Wise -</option>');
	//foreach($rows as $r){
