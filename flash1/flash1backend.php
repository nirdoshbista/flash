...
if ($req=='schoollist_e'){
	...
	//schools entered
	if ($order=='code')
		$result=mysql_query("select distinct sch_num, nm_sch, closed, merged_with from mast_schoollist where dist_code='$dist' and vdc_code='$vdc' and sch_year='$currentyear' order by sch_num");
	else 
		$result=mysql_query("select distinct sch_num, nm_sch, closed, merged_with from mast_schoollist where dist_code='$dist' and vdc_code='$vdc' and sch_year='$currentyear' order by nm_sch");
...
if ($req=='schoollist_ne'){
	...
	foreach($schools as $s){
		...
		$result=mysql_query("select * from mast_schoollist where sch_year='$currentyear' and sch_num='${s['sch_num']}'");
