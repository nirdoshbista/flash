...
<p align="center">Note: Only schools entered in Flash I can be edited.</p>
<?php
...
if (isset($_GET['req'])){
	...
	if ($_GET['req']=='edit'){
		...
		$query="update mast_schoollist set nm_sch='$sname' where dist_code='$d' and vdc_code='$v' and sch_num='$s' and sch_year='$year'";
...
<?php
	...
	$result=mysql_query("select * from mast_schoollist where dist_code='".$_GET['distcode']."' and vdc_code='".$_GET['vdccode']."' and sch_year='".$_GET['year']."' order by sch_num");
	$rows = mysql_fetch_all($result);
