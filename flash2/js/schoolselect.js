...
function initialize(){
	ajaxRequest('flash2backend.php?req=distlist',distCallback);
...
function districtChange(){
	...
	ajaxRequest('flash2backend.php?req=vdclist&distcode='+document.getElementById('distlist').value, vdcCallback);
...
function vdcChange(){
	...
	if (currentSchoolCode=='') ajaxText('flash2backend.php?req=newschoolcode&distcode='+document.getElementById('distlist').value+'&vdccode='+document.getElementById('vdclist').value, 'sch_code');

	var o='';
	if (document.getElementById('ordername').checked) o='name';
	if (document.getElementById('ordercode').checked) o='code';
	
	ajaxDiv('flash2backend.php?req=schoollist_e&distcode='+document.getElementById('distlist').value+'&vdccode='+document.getElementById('vdclist').value+'&order='+o, 'schoollist_e');
	ajaxDiv('flash2backend.php?req=schoollist_ne&distcode='+document.getElementById('distlist').value+'&vdccode='+document.getElementById('vdclist').value+'&order='+o, 'schoollist_ne');
	
	/*
	ajaxRequest('flash2backend.php?req=schoolsuggest&distcode='+document.getElementById('distlist').value+'&vdccode='+document.getElementById('vdclist').value, 
		function(t){
...
function loadSchool(a){
	...
	// load page1
	ajaxRequest('flash2backend.php?req=loadschool&schoolcode='+a, loadSchoolMain);
...
function schoolSelect(o){
	...
	if (o.id=='slist_e') {
		document.getElementById('slist_ne').selectedIndex = '-1';
		document.getElementById('sch_close').disabled = true;
		document.getElementById('sch_merge').disabled = true;
	}
	if (o.id=='slist_ne') {
		document.getElementById('slist_e').selectedIndex = '-1';
		document.getElementById('sch_close').disabled = false;
		document.getElementById('sch_merge').disabled = false;
	}
	
	document.getElementById('s').value = o.value;
	
	if (o.options[o.selectedIndex].className=='closedschool')
		document.getElementById('submitbtn').disabled = true;
	else
		document.getElementById('submitbtn').disabled = false;
		
	if (o.id=='slist_e' && document.getElementById('merge_with').value!=''){
		var url = 'flash2backend.php?req=sch_merge&s=' + document.getElementById('merge_with').value + '&mw=' + o.value;
		document.getElementById('slist_ne').disabled = false;
		document.getElementById('merge_with').value = '';
...
function schoolClose(){
	var index = document.getElementById('slist_ne').selectedIndex;
	if (confirm('Do you want to close '+ document.getElementById('slist_ne').options[index].text) + '?'){
		
		ajaxRequest('flash2backend.php?req=sch_close&s='+document.getElementById('s').value,function(d){
			vdcChange();
		});
...
function schoolMerge(){
	document.getElementById('merge_with').value = document.getElementById('slist_ne').value;
	document.getElementById('slist_ne').disabled = true;
	
	alert("Now select school to merge with under Entered Schools list");
}
