...
if ($req=='schoollist'){
	...
	if ($vdc==0){
		...
		$result = mysql_query(sprintf('select *, TRIM(nm_sch) as schoolname from mast_schoollist where dist_code="%s" and vdc_code="%s" and sch_year="%s" and flash1="1" and flash2="1" order by schoolname',$dist, $vdc,$yr));
		$rows = mysql_fetch_all($result);
