...
if ($req == 'schoollist_e' || $req == 'schoollist_ne'){
	...
	$result = mysql_query("SELECT DISTINCT mast_schoollist.sch_num as sch_num, nm_sch FROM mast_schoollist LEFT JOIN mast_school_type USING (sch_num, sch_year) WHERE dist_code='$dist' AND vdc_code='$vdc' AND class$class AND sch_year='$currentyear' AND mast_schoollist.flash1='1' ORDER BY $order",$flashdblink);
	...
	foreach($schools as $s){
		...
		$result = mysql_query("SELECT count(stu_num) as n FROM student_main WHERE sch_num LIKE '{$s['sch_num']}%'");
